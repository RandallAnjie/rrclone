...
func TestTokenFileMapper(t *testing.T) {
	...
	t.Run("Structured", func(t *testing.T) {
		...
		// Missing expiry is normalized so oauth2 treats the access
		// token as expired and refreshes instead of reusing it.
		v, ok = m.Get("token")
		assert.True(t, ok)
		assert.Equal(t, `{"access_token":"newtok","expiry":"1970-01-01T00:00:00Z","token_type":"Bearer"}`, v)
	...
	t.Run("StructuredMissingCredentials", func(t *testing.T) {
		...
		// Token is extracted from the "token" field and missing
		// expiry is normalized to force a refresh.
		v, ok := m.Get("token")
		assert.True(t, ok)
		assert.Equal(t, `{"access_token":"tok","expiry":"1970-01-01T00:00:00Z","token_type":"Bearer"}`, v)
