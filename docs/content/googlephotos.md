...
rclone talks to Google Photos at `https://photoslibrary.googleapis.com/v1`.
If that host is blocked, set `endpoint` to a reverse proxy of it.

OAuth uses different hosts. Override those with the existing
`auth_url` and `token_url` options. Photo bytes may still be served
from other Google hosts (for example `googleusercontent.com`); those
are not covered by `endpoint`.
...
endpoint = https://photoslibrary.example.com
```
